...
def compute_persistence_landscape(diag, dimension, scale_range=None, k=50, samples=50):
...
# persistence images
def compute_persistence_image(bd_pairs, out_res, max_dist, max_persistence, sigma=0.05, boundary_mode='hard'):

    assert(boundary_mode in ['soft', 'hard'])

    is_dim0 = np.all(bd_pairs[:, 0] == 0)

    def weight_function(bp_pairs, max_persistence):

        w = np.zeros((bp_pairs.shape[0], 1))
        w = bp_pairs[:, 1] / np.double(max_persistence)
        w[w > 1] = 1
    ...
    w = weight_function(bp_pairs, max_persistence)
    ...
    if is_dim0:
        ...
        for pid in range(bp_pairs.shape[0]):
            ...
            out_image += w[pid] * _compute_mvnorm_image_dim_1(ubound, out_res, bp_pairs[pid, 1], sigma)
        ...
        for pid in range(bp_pairs.shape[0]):
            ...
            cur_out_image = dask.delayed(_compute_mvnorm_image_dim_2)(ubound, out_res, bp_pairs[pid, :], sigma)
            out_image_list.append(cur_out_image)

        out_image_list = dask.delayed(out_image_list).compute()

        out_image = 0
        for pid in range(bp_pairs.shape[0]):
            out_image += w[pid] * out_image_list[pid]
...
def _compute_mvnorm_image_dim_2(upper_bound, out_res, mu, sigma):
    ...
    for i in range(out_res):
        for j in range(out_res):
            ...
            out_image[j, i] = mvnormcdf([x_vals[i+1], y_vals[j+1]], mu, sigma, lower=[x_vals[i], y_vals[j]])
