...
class ResNetRGBTrainer(BaseTrainer):
    def __init__(self, model, config):
        super(ResNetRGBTrainer, self).__init__(model, config)
    ...
    def trainModel(self, TrainGenerator, CVGenerator):
        ...
        len_train = len(TrainGenerator.train_paths)
        len_CV = len(CVGenerator.cv_paths)
    ...
    def train(self, TrainGenerator, CVGenerator):
        ...
        def print_params():
            ...
            print '# CV Malignant    : ', len(CVGenerator.mal_paths_cv)
            print '# CV Benign       : ', len(CVGenerator.ben_paths_cv)
...
def ResNetRGBTest(model, data, config):
    X_test, Y_test = data

    metrics_vals = model.evaluate(X_test , to_categorical(Y_test, num_classes=2) , batch_size=config.trainer.batch_size)
